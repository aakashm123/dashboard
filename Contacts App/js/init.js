...
document.getElementById('btn-sub').addEventListener('click', function(event) {
		// 1. Prevent the default browser form submission (prevents page refresh)
		event.preventDefault();

		// 2. Fetch data from input fields
		const nameInput = document.getElementById('un').value;
		const emailInput = document.getElementById('em').value;
		const phoneInput = document.getElementById('ph').value;
		const designationInput = document.getElementById('ds').value;
		const ageInput = document.getElementById('ag').value;
		const dojInput = document.getElementById('dj').value;
		const salaryInput = document.getElementById('sl').value;


		// 3. Inject the text values into the target display elements
		// document.getElementById('displayName').textContent = nameInput;
		// document.getElementById('displayEmail').textContent = emailInput;

		const tableBody = document.querySelector("#datable_1 tbody");

		// Define your new row HTML structure
		const rowHTML = `
		<tr>
			<td>1</td>
			<td><a href="#">${nameInput}</a></td>
			<td>${emailInput}</td>
			<td>${phoneInput}</td>
			<td><span class="label label-danger">${designationInput}</span> </td>
			<td>${dojInput}</td>
			<td>${salaryInput}</td>
		...
		// Append the HTML string right before the end of the tbody
		tableBody.insertAdjacentHTML('beforeend', rowHTML);
		// 4. Make the hidden output container visible
...
// csv uploaded
	//	var file = document.getElementById('file');
		...
		//         let CSVARRAY = parseResult(e.target.result); //this is where the csv array will be
		// 				console.log(CSVARRAY);
		// 				// 1. Select the table body
		// 				const tbody = document.querySelector("#datable_1 tbody");
		//
		// 				// 2. Insert a new row at the very end of the tbody
		...
		//
		// 					}
		// 				// document.getElementById("datable_1").innerHTML = generateTableFrom2DArray(CSVARRAY);
		//
		//
		//
		//     };
		...
		//     reader.readAsText(f);
		// });
// 		function generateTableFrom2DArray(arrayData) {
// 			// Map rows to <tr> elements, and individual items to <td> elements
// 			const cleanarray = arrayData;
// 			cleanarray.shift();
// 			const tableRows = cleanarray.map((row, index) => {
// 				const cellTag = index === 0 ? 'td' : 'td'; // Makes the first row a Header row
// 				const cells = row.map(cell => `<${cellTag}>${cell}</${cellTag}>`).join('');
// 				return `<tr>${cells}<td><a href="javascript:void(0)" class="text-inverse pr-10" title="Edit" data-toggle="tooltip"><i class="zmdi zmdi-edit txt-warning"></i></a><a href="javascript:void(0)" class="text-inverse" title="Delete" data-toggle="tooltip"><i class="zmdi zmdi-delete txt-danger"></i></a></td>
// 		</tr>`;
// 			}).join('\n');
//
